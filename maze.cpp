...
void maze::condition1(int*j,int*k)
{
    if(*k==390)
    {
        if(*j>=10 && *j<=500)
...
void maze::condition2(int *j,int *k)
{
    if(*k==110)
    {
        if(*j>=10&&*j<=440)
...
void maze::condition3(int *j,int *k)
{  if(*k==110){
	   if(*j<450 && *j>0)
	   *k=*k+5;
	   }
...
void maze::caption()
{
  ...
  outtextxy(10,50,"Press Esc to Escape from the Maze");
  setcolor(WHITE);
}
void maze::level_1()
{
